"""
auth/user_operations.py
========================
CRUD operations for the `users` and `audit_log` collections.

Works with BOTH MongoDB and Supabase depending on DB_BACKEND env var.
All password hashing uses bcrypt with cost factor 12.
Passwords are NEVER stored, logged, or returned in any function.
"""

from __future__ import annotations

import logging
import os
import re
import uuid
from datetime import datetime
from typing import Any

import bcrypt

import config

logger = logging.getLogger(__name__)

_backend = os.environ.get("DB_BACKEND", "mongo").strip().lower()


def _get_supabase():
    """Return the Supabase client (only used when DB_BACKEND=supabase)."""
    from database.supabase_client import get_supabase
    return get_supabase()


def _get_mongo_db():
    """Return the MongoDB db handle (only used when DB_BACKEND=mongo)."""
    from database.mongo_client import get_db
    return get_db()

logger = logging.getLogger(__name__)


# ─────────────────────────────────────────────────────────────────────────────
# Helpers
# ─────────────────────────────────────────────────────────────────────────────

def _hash_password(plaintext: str) -> str:
    """Hash a plaintext password with bcrypt (cost=12). Returns the hash string."""
    return bcrypt.hashpw(
        plaintext.encode("utf-8"),
        bcrypt.gensalt(rounds=12),
    ).decode("utf-8")


def _make_user_id(role: str, username: str) -> str:
    """Generate a deterministic-ish user_id. Example: 'S-24adr064', 'T-john'."""
    prefix = {"student": "S", "teacher": "T", "admin": "A"}.get(role, "U")
    return f"{prefix}-{username[:20]}"


# ─────────────────────────────────────────────────────────────────────────────
# User CRUD
# ─────────────────────────────────────────────────────────────────────────────

def create_user(
    username: str,
    password: str,
    role: str,
    full_name: str,
    email: str = "",
    linked_student_id: str | None = None,
    assigned_sections: list[str] | None = None,
    created_by: str = "system",
) -> tuple[bool, str]:
    """
    Create a new user account.
    Returns (True, user_id) on success, (False, error_message) on failure.
    """
    username = username.strip()
    role     = role.strip().lower()

    if not username:
        return False, "Username is required."
    if role not in config.VALID_ROLES:
        return False, f"Invalid role '{role}'. Must be one of: {config.VALID_ROLES}."
    if len(password) < config.PASSWORD_MIN_LENGTH:
        return False, f"Password must be at least {config.PASSWORD_MIN_LENGTH} characters."

    user_id = _make_user_id(role, username)

    if _backend == "supabase":
        sb = _get_supabase()
        # Check duplicate username
        existing = sb.table("users").select("username").ilike("username", username).execute()
        if existing.data:
            return False, f"Username '{username}' is already taken."
        # Check duplicate user_id
        if sb.table("users").select("user_id").eq("user_id", user_id).execute().data:
            user_id = f"{user_id}-{uuid.uuid4().hex[:4]}"
        # Validate linked student
        if role == "student" and linked_student_id:
            s = sb.table("students").select("student_id").eq("student_id", linked_student_id).execute()
            if not s.data:
                return False, f"Student ID '{linked_student_id}' not found."
        doc = {
            "user_id":           user_id,
            "username":          username,
            "password_hash":     _hash_password(password),
            "role":              role,
            "is_active":         True,
            "student_id":        linked_student_id,
            "created_on":        datetime.utcnow().isoformat(),
            "last_login":        None,
            "failed_attempts":   0,
            "locked_until":      None,
        }
        sb.table("users").insert(doc).execute()
    else:
        db = _get_mongo_db()
        if db["users"].find_one({"username": {"$regex": f"^{re.escape(username)}$", "$options": "i"}}):
            return False, f"Username '{username}' is already taken."
        if db["users"].find_one({"user_id": user_id}):
            user_id = f"{user_id}-{uuid.uuid4().hex[:4]}"
        if role == "student" and linked_student_id:
            if not db["students"].find_one({"student_id": linked_student_id}):
                return False, f"Student ID '{linked_student_id}' not found."
        doc = {
            "user_id":           user_id,
            "username":          username,
            "password_hash":     _hash_password(password),
            "role":              role,
            "full_name":         full_name.strip(),
            "email":             email.strip(),
            "linked_student_id": linked_student_id,
            "assigned_sections": assigned_sections or [],
            "is_active":         True,
            "created_on":        datetime.utcnow(),
            "created_by":        created_by,
            "last_login":        None,
            "login_attempts":    0,
            "lockout_until":     None,
            "force_password_reset": False,
        }
        db["users"].insert_one(doc)

    logger.info("Created user %s (role=%s).", username, role)
    return True, user_id


def get_user_by_username(username: str) -> dict | None:
    """Return a user document by username (case-insensitive), or None."""
    if _backend == "supabase":
        sb = _get_supabase()
        res = sb.table("users").select("*").ilike("username", username.strip()).execute()
        return res.data[0] if res.data else None
    else:
        db = _get_mongo_db()
        return db["users"].find_one(
            {"username": {"$regex": f"^{re.escape(username.strip())}$", "$options": "i"}},
            {"_id": 0},
        )


def get_user_by_id(user_id: str) -> dict | None:
    """Return a user document by user_id, or None."""
    if _backend == "supabase":
        sb = _get_supabase()
        res = sb.table("users").select("*").eq("user_id", user_id).execute()
        return res.data[0] if res.data else None
    else:
        db = _get_mongo_db()
        return db["users"].find_one({"user_id": user_id}, {"_id": 0})


def get_all_users(role_filter: str | None = None) -> list[dict]:
    """Return all user documents (password_hash excluded). Optionally filter by role."""
    if _backend == "supabase":
        sb = _get_supabase()
        q = sb.table("users").select("user_id,username,role,is_active,student_id,created_on,last_login")
        if role_filter:
            q = q.eq("role", role_filter.lower())
        res = q.order("created_on", desc=True).execute()
        return res.data or []
    else:
        db = _get_mongo_db()
        query: dict[str, Any] = {}
        if role_filter:
            query["role"] = role_filter.lower()
        return list(
            db["users"].find(query, {"_id": 0, "password_hash": 0}).sort("created_on", -1)
        )


def update_user(user_id: str, fields: dict, actor_user_id: str = "system") -> bool:
    """Update arbitrary user fields. Returns True if found and updated."""
    fields.pop("password_hash", None)
    fields.pop("user_id", None)
    if _backend == "supabase":
        sb = _get_supabase()
        res = sb.table("users").update(fields).eq("user_id", user_id).execute()
        return bool(res.data)
    else:
        db = _get_mongo_db()
        result = db["users"].update_one({"user_id": user_id}, {"$set": fields})
        return result.matched_count > 0


def deactivate_user(user_id: str, actor_user_id: str) -> bool:
    """Deactivate a user account. Returns True if found."""
    if _backend == "supabase":
        sb = _get_supabase()
        user_res = sb.table("users").select("username").eq("user_id", user_id).execute()
        if not user_res.data:
            return False
        sb.table("users").update({"is_active": False}).eq("user_id", user_id).execute()
    else:
        db = _get_mongo_db()
        user = db["users"].find_one({"user_id": user_id}, {"_id": 0})
        if not user:
            return False
        db["users"].update_one({"user_id": user_id}, {"$set": {"is_active": False}})
    logger.info("User %s deactivated by %s.", user_id, actor_user_id)
    return True


def reactivate_user(user_id: str, actor_user_id: str) -> bool:
    """Reactivate a previously deactivated user account."""
    if _backend == "supabase":
        sb = _get_supabase()
        user_res = sb.table("users").select("user_id").eq("user_id", user_id).execute()
        if not user_res.data:
            return False
        sb.table("users").update({"is_active": True, "failed_attempts": 0, "locked_until": None}).eq("user_id", user_id).execute()
    else:
        db = _get_mongo_db()
        user = db["users"].find_one({"user_id": user_id}, {"_id": 0})
        if not user:
            return False
        db["users"].update_one(
            {"user_id": user_id},
            {"$set": {"is_active": True, "login_attempts": 0, "lockout_until": None}},
        )
    return True


def reset_password(user_id: str, new_password: str, actor_user_id: str) -> tuple[bool, str]:
    """Admin resets a user's password."""
    if len(new_password) < config.PASSWORD_MIN_LENGTH:
        return False, f"Password must be at least {config.PASSWORD_MIN_LENGTH} characters."
    new_hash = _hash_password(new_password)
    if _backend == "supabase":
        sb = _get_supabase()
        res = sb.table("users").update({"password_hash": new_hash, "failed_attempts": 0, "locked_until": None}).eq("user_id", user_id).execute()
        if not res.data:
            return False, f"User ID '{user_id}' not found."
    else:
        db = _get_mongo_db()
        result = db["users"].update_one(
            {"user_id": user_id},
            {"$set": {"password_hash": new_hash, "force_password_reset": True, "login_attempts": 0, "lockout_until": None}},
        )
        if result.matched_count == 0:
            return False, f"User ID '{user_id}' not found."
    logger.info("Password reset for user %s by admin %s.", user_id, actor_user_id)
    return True, ""


def change_password(user_id: str, old_password: str, new_password: str) -> tuple[bool, str]:
    """User changes their own password (requires current password verification)."""
    user = get_user_by_id(user_id)
    if not user:
        return False, "User not found."
    try:
        ok = bcrypt.checkpw(old_password.encode("utf-8"), user["password_hash"].encode("utf-8"))
    except Exception:
        ok = False
    if not ok:
        return False, "Current password is incorrect."
    if len(new_password) < config.PASSWORD_MIN_LENGTH:
        return False, f"New password must be at least {config.PASSWORD_MIN_LENGTH} characters."
    new_hash = _hash_password(new_password)
    if _backend == "supabase":
        sb = _get_supabase()
        sb.table("users").update({"password_hash": new_hash, "failed_attempts": 0}).eq("user_id", user_id).execute()
    else:
        db = _get_mongo_db()
        db["users"].update_one({"user_id": user_id}, {"$set": {"password_hash": new_hash, "force_password_reset": False}})
    return True, ""


def delete_user(user_id: str, actor_user_id: str) -> bool:
    """Permanently delete a user account. Returns True if found and deleted."""
    if _backend == "supabase":
        sb = _get_supabase()
        res = sb.table("users").delete().eq("user_id", user_id).execute()
        return bool(res.data)
    else:
        db = _get_mongo_db()
        user = db["users"].find_one({"user_id": user_id}, {"_id": 0})
        if not user:
            return False
        db["users"].delete_one({"user_id": user_id})
        return True


# ─────────────────────────────────────────────────────────────────────────────
# Audit Log
# ─────────────────────────────────────────────────────────────────────────────

def log_audit(
    actor_user_id: str,
    actor_role: str = "system",
    action: str = "",
    target: str = "",
    old_value: str = "",
    new_value: str = "",
) -> None:
    """Write an audit log entry."""
    if _backend == "supabase":
        sb = _get_supabase()
        sb.table("audit_log").insert({
            "actor_user_id": actor_user_id,
            "action":        action,
            "target_type":   actor_role,
            "target_id":     target,
            "details":       {"old": old_value, "new": new_value},
            "timestamp":     datetime.utcnow().isoformat(),
        }).execute()
    else:
        db = _get_mongo_db()
        db["audit_log"].insert_one({
            "actor_user_id": actor_user_id,
            "actor_role":    actor_role,
            "action":        action,
            "target":        target,
            "old_value":     old_value,
            "new_value":     new_value,
            "timestamp":     datetime.utcnow(),
        })
    logger.info("AUDIT | actor=%s action=%s target=%s", actor_user_id, action, target)


def get_audit_log(
    actor_user_id: str | None = None,
    action: str | None = None,
    date_from: datetime | None = None,
    date_to: datetime | None = None,
    limit: int = 200,
) -> list[dict]:
    """Query audit_log with optional filters. Returns newest-first."""
    if _backend == "supabase":
        sb = _get_supabase()
        q = sb.table("audit_log").select("*").order("timestamp", desc=True).limit(limit)
        if actor_user_id:
            q = q.eq("actor_user_id", actor_user_id)
        if action:
            q = q.eq("action", action)
        res = q.execute()
        return res.data or []
    else:
        db = _get_mongo_db()
        query: dict[str, Any] = {}
        if actor_user_id:
            query["actor_user_id"] = actor_user_id
        if action:
            query["action"] = action
        if date_from or date_to:
            ts_filter: dict[str, Any] = {}
            if date_from:
                ts_filter["$gte"] = date_from
            if date_to:
                ts_filter["$lte"] = date_to
            query["timestamp"] = ts_filter
        records = list(
            db["audit_log"].find(query, {"_id": 0}).sort("timestamp", -1).limit(limit)
        )
        for r in records:
            if isinstance(r.get("timestamp"), datetime):
                r["timestamp"] = r["timestamp"].strftime("%Y-%m-%d %H:%M:%S UTC")
        return records
